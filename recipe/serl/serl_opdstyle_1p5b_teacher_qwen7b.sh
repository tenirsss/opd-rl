# 保留serl的逻辑，将opsd换成opd，只是换teacher：teacher 7B instruct ：student = Qwen2.5-1.5B-Instruct，teacher = GiGPO-Qwen2.5-7B-Instruct-ALFWorld
#!/usr/bin/env bash
set -euo pipefail

# SERL method with an OPD-style fixed external teacher:
# student = Qwen2.5-1.5B-Instruct, teacher = Qwen2.5-7B-Instruct.
...
OUTPUT_ROOT=${OUTPUT_ROOT:-$REPO_DIR/outputs/serl_opdstyle_1p5b_teacher_qwen7b}
